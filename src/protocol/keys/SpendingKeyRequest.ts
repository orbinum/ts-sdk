...
import { PRECOMPILE_ADDR } from '../../chain/evm/precompiles/addresses';
...
/**
 * The web origin sr25519 spending keys are bound to.
 *
 * The sr25519 route signs in the `substrate-vrf` namespace, whose effective
 * context includes the ORIGIN the wallet was asked from. That binding is the
 * security property — a hostile site asking for the same payload derives a
 * different, useless identity — but it also means the origin is part of the
 * derivation: an sr25519 identity created at one origin cannot be reproduced at
 * another.
 *
 * Stated here, next to `SPENDING_KEY_VERIFYING_CONTRACT`, because it plays the
 * same role for sr25519 that the verifying contract plays for EVM: a wallet
 * that does not know this value derives an identity Orbinum will not recognise.
 *
 * Changing it rotates every sr25519 identity ever derived.
 */
export const SPENDING_KEY_CANONICAL_ORIGIN = 'https://app.orbinum.network';
