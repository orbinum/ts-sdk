...
import { deriveSelfEphSk, selfEphWindow } from '../../src/shielded-pool/protocol/selfEph';
...
function asHint(note: { commitmentHex: string; memo: number[] }, leafIndex = 0): ScanCommitment {
  return { commitmentHex: note.commitmentHex, leafIndex, encryptedMemo: toHex(new Uint8Array(note.memo)) };
}
