...
import { deriveVaultKey, encryptJson, decryptJson } from '../../../../src/wallet/vault/crypto/keys';
import { vaultReplacer, vaultReviver } from '../../../../src/wallet/vault/crypto/json';
...
describe('encryptJson / decryptJson', () => {
    ...
    it('roundtrips nested bigint values', async () => {
        ...
        const { iv, ciphertext } = await encryptJson(key, payload);
        const result = (await decryptJson(key, iv, ciphertext)) as {
            keys: { a: bigint; b: bigint };
            note: null;
        };
