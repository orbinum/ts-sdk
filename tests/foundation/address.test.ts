...
describe('addressToFieldElement', () => {
    // The mapping is circuit-defined: H160 ++ [0x00; 12], bytes read
    // little-endian, reduced mod BN254_R. A wrong scalar makes the proof verify
    // against a DIFFERENT recipient — funds go to whoever that value names.
    it('maps an EVM address as H160 zero-padded little-endian', () => {
    ...
    it('maps an SS58 address through its decoded AccountId32', () => {
        ...
        const accountIdHex = addressToAccountIdHex(ss58)!;
...
/**
 * `parseEvmAddress` — la variante que comprueba.
 *
 * `normalizeEvmAddress` reformatea lo que le den: `'hello'` sale como
 * `'0xhello'`. Es correcto cuando quien llama ya sabe que es una dirección, y
 * peligroso en un campo de formulario — de ahí que cada host acabara
 * escribiendo este wrapper por su cuenta.
 */
describe('parseEvmAddress', () => {
    const ADDR = '0x1234567890abcdef1234567890abcdef12345678';

    it('normaliza una dirección válida', () => {
        expect(parseEvmAddress(ADDR.toUpperCase().replace('0X', '0x'))).toBe(ADDR);
        expect(parseEvmAddress(ADDR.slice(2))).toBe(ADDR);
    });

    it('rechaza lo que no es una dirección, donde normalizeEvmAddress no', () => {
        // La diferencia que justifica que exista: la forma sin comprobación
        // devuelve una cadena con pinta de dirección para cualquier basura.
        expect(normalizeEvmAddress('hello')).toBe('0xhello');
        expect(parseEvmAddress('hello')).toBeNull();
    });
