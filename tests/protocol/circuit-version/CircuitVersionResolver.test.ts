...
} from '../../../src/protocol/circuit-version/index';
import type { ZkVerifierModule } from '../../../src/chain/pallet/zk-verifier/ZkVerifierModule';
import type { ZkVerifierCircuitVersionInfo } from '../../../src/chain/pallet/zk-verifier/types/client';
...
describe('CircuitVersionResolver', () => {
    ...
    it('throws when the chain does not support the note version', async () => {
        ...
        const resolver = makeResolver(info, fakeProvider(1, VK_V1));
        await expect(resolver.resolve(CircuitType.Unshield, 1)).rejects.toThrow(/does not support/);
    });
    ...
    it('throws on vk_hash mismatch between prover and chain', async () => {
        ...
        await expect(resolver.resolve(CircuitType.Unshield, 1)).rejects.toThrow(/VK hash mismatch/);
    });
    ...
    it('throws when the prover resolves a different version than requested', async () => {
        ...
        await expect(resolver.resolve(CircuitType.Unshield, 1)).rejects.toThrow(/prover resolved/);
    });

    it('throws when the chain has no version info for the circuit', async () => {
        const resolver = makeResolver(null, fakeProvider(1, VK_V1));
        await expect(resolver.resolve(CircuitType.Unshield, 1)).rejects.toThrow(/no version info/);
    });
