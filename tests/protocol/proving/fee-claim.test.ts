...
import type { ArtifactProvider, ProofResult } from '../../../src/protocol/proving/unshield';
...
vi.mock('@orbinum/proof-generator', () => ({
    ...
    generateProof: vi.fn(),
    WebArtifactProvider: vi.fn().mockImplementation(function (this: object) {
        return this;
    }),
}));
...
describe('generateFeeClaimProof', () => {
    ...
    it('throws when amount is 0n', async () => {
        await expect(generateFeeClaimProof({ ...BASE_INPUTS, amount: 0n })).rejects.toThrow(
            /amount must be greater than zero/
        );
    });

    it('throws when amount is negative', async () => {
        await expect(generateFeeClaimProof({ ...BASE_INPUTS, amount: -1n })).rejects.toThrow(
            /amount must be greater than zero/
        );
    });
