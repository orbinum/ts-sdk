...
import {
    ...
    type NoteDisclosure,
} from '../../../src/protocol/note/NoteDisclosure';
...
describe('createNoteDisclosureKey', () => {
    ...
    it('produces different keys for different values', () => {
        const a = createNoteDisclosureKey(
            makeNote({ value: 100n, commitment: poseidon4([100n, 0n, 123456789n, 987654321n]) })
        );
        const b = createNoteDisclosureKey(
            makeNote({ value: 200n, commitment: poseidon4([200n, 0n, 123456789n, 987654321n]) })
        );
        expect(a).not.toBe(b);
    });
    ...
    it('produces different keys for different assetIds', () => {
        const a = createNoteDisclosureKey(
            makeNote({
                assetId: 0n,
                commitment: poseidon4([1_000_000n, 0n, 123456789n, 987654321n]),
            })
        );
        const b = createNoteDisclosureKey(
            makeNote({
                assetId: 1n,
                commitment: poseidon4([1_000_000n, 1n, 123456789n, 987654321n]),
            })
        );
        expect(a).not.toBe(b);
    });
    ...
    it('produces different keys for different ownerPks', () => {
        const a = createNoteDisclosureKey(
            makeNote({ ownerPk: 1n, commitment: poseidon4([1_000_000n, 0n, 1n, 987654321n]) })
        );
        const b = createNoteDisclosureKey(
            makeNote({ ownerPk: 2n, commitment: poseidon4([1_000_000n, 0n, 2n, 987654321n]) })
        );
        expect(a).not.toBe(b);
    });
...
describe('portability', () => {
    ...
    it('round-trips a key without btoa or atob', () => {
        ...
        const decoded = decodeNoteDisclosureKey(createNoteDisclosureKey(note));
    ...
    it('produces the SAME key a btoa-capable runtime does', () => {
        ...
        const note = makeNote();
        const withoutGlobals = createNoteDisclosureKey(note);

        globalThis.btoa = saved.btoa;
        globalThis.atob = saved.atob;

        expect(createNoteDisclosureKey(note)).toBe(withoutGlobals);
    });
});
...
describe('createNoteDisclosureKey / decodeNoteDisclosureKey round-trip', () => {
    ...
    it('different notes produce different disclosure keys', () => {
        const noteA = makeNote({
            value: 100n,
            commitment: poseidon4([100n, 0n, 123456789n, 987654321n]),
        });
        const noteB = makeNote({
            value: 101n,
            commitment: poseidon4([101n, 0n, 123456789n, 987654321n]),
        });
    ...
    it('decoding noteA key with noteB data fails verification (cross-note forgery)', () => {
        ...
        const noteB = makeNote({
            value: 200n,
            commitment: poseidon4([200n, 0n, 123456789n, 987654321n]),
        });
