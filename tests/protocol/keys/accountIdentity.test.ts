...
import { deriveSpendingKeyMessageV2 } from '../../../src/protocol/keys/SpendingKeyRequest';
...
/**
 * A byte-varied 65-byte signature. Derivation rejects a placeholder carrying
 * almost no distinct bytes, so a repeated pattern would exercise the rejection
 * path instead of the identity-stability property under test.
 */
const SIG =
    '0x' +
    Array.from({ length: 65 }, (_, i) => ((i * 61 + 13) & 0xff).toString(16).padStart(2, '0')).join(
        ''
    );
const CHAIN_ID = 2700;
