...
describe('pkm.importFromHex', () => {
    ...
    it('round-trips through exportHex → importFromHex', async () => {
        ...
        // Copy: getters return the live buffer and clear() now zeroes it, so a
        // reference captured across a clear() would read as zeros. A consumer
        // that needs a value past a clear() must copy it — as here.
        const vk = pkm.getViewingSecretKey().slice();
        const pk = pkm.getOwnerPk();
