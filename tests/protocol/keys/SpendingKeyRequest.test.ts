...
} from '../../../src/protocol/keys/SpendingKeyRequest';
...
describe('SPENDING_KEY_VERIFYING_CONTRACT', () => {
    ...
    it('pins the on-chain precompile address (2049 → 0x…0801)', () => {
        expect(SPENDING_KEY_VERIFYING_CONTRACT).toBe('0x0000000000000000000000000000000000000801');
    });
});
...
describe('v1 derivation is gone', () => {
    it('SECURITY: the module exports no v1 builder', async () => {
        const mod = await import('../../../src/protocol/keys/SpendingKeyRequest');
    ...
    it('SECURITY: no builder emits the v1 domain tag', () => {
        expect(deriveSpendingKeyMessageV2(CHAIN_ID, ADDR)).not.toContain('orbinum-spending-key-v1');
